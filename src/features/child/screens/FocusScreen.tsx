...
import type { StudyLockPresentation } from '../studyLockState';
...
interface FocusScreenProps {
  ...
  maxBreaks: number;
  note: string;
  lockState: StudyLockPresentation;
...
export function FocusScreen({
  ...
  const saveNote = async () => {
    ...
    try {
      ...
      setActionError(childActionError(cause, 'Chưa lưu được ghi chú. Hãy thử lại.'));
    }
  };
  ...
  const completeSession = async () => {
    ...
    try {
      ...
      setActionError(childActionError(cause, 'Chưa hoàn tất được buổi học. Hãy thử lại.'));
    }
  };

  return (
    <section className="focus-screen child-full-screen">
      ...
      <div className="focus-main">
        <section
          className={`focus-lock-banner is-${lockState.tone}`}
          aria-labelledby="focus-lock-state"
          aria-live="polite"
        >
          <span id="focus-lock-state"><MaterialIcon name="shield" />{lockState.label}</span>
